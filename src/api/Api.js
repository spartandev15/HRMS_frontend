...
const GET_ALL_LEAVES = async (status) => {
  return axios
    .get(`${BASE_URL}/api/get/all/leaves?status=${status}`,  {
      headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
    })
    .then((res) => {
      return res;
...
const UPDATE_LEAVE_STATUS = async (data) => {
  return axios
    .post(`${BASE_URL}/api/change/leave/status`, data,  {
      headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
    })
    .then((res) => {
      return res;
